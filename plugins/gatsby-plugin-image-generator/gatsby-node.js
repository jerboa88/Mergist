...
async function createDir(outputImgPath) {
	return new Promise((resolve, reject) => {
		...
		fs.promises
			.mkdir(dirname(outputImgPath), { recursive: true })
			.then(resolve)
			.catch((err) => {
...
async function copyImg(inputImgPath, outputImgPath) {
	return new Promise(async (resolve, reject) => {
		...
		fs.promises
			.copyFile(inputImgPath, outputImgPath)
			.then(resolve)
			.catch((err) => {
				reject(err);
			});
...
async function generateRasterImg(
	inputImg,
	outputImgWidth,
	outputImgHeight,
	outputImgPath,
...
async function processInputImgRule({ from, to: outputImgRules, options }) {
	return new Promise(async (resolve, reject) => {
		debug(
			`Processing input image rule: ${from} -> ${outputImgRules.length} output ${pluralize(outputImgRules.length)}`,
			1,
		);
...
async function processOutputImgRule(
	inputImgPath,
	inputImg,
	{ path, size },
	options,
	...
	return new Promise(async (resolve, reject) => {
		...
		const [outputImgWidth, outputImgHeight] =
			size.length === 2 ? size : [size[0], size[0]];

		debug(
			`Processing output image rule: ${path} (${outputImgWidth}x${outputImgHeight})`,
			2,
		);

		// Create the output folder if it doesn't exist
		createDir(outputImgPath)
			.then(async () => {
				if (isSvg(outputImgPath)) {
					...
					if (!isSvg(inputImgPath)) {
						throw new Error(
							`Cannot convert a raster input image ${inputImgPath} to an SVG`,
						);
					}
...
exports.onPostBootstrap = async (
	...
	await Promise.all(promiseArray)
		.then((values) => {
			...
			const numOfOutputImgs = values.reduce(
				(previousValue, currentValue) => previousValue + currentValue,
			);

			reporter.info(
				`${pluginName} generated ${numOfOutputImgs} output ${pluralize(numOfOutputImgs)} from ${numOfInputImgs} input ${pluralize(numOfInputImgs)}`,
			);
			activity.end();
		})
		.catch((err) => {
			reporter.error(
				'Something went wrong while generating output images:',
				err,
			);
		});
};
