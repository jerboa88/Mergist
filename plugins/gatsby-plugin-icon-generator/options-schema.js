...
exports.pluginOptionsSchema = ({ Joi }) => {
	...
	// Schemas
	const svgFileSchema = Joi.string().required().pattern(/\.svg$/i);
	const rasterFileSchema = Joi.string().required().pattern(/\.(?:jpg|jpeg|png|webp|gif|avif|tif|tiff)$/i);
	const sizeSchema = Joi.array().required().single().min(1).max(2).items(
		Joi.number().required().min(minImgSize).max(maxImgSize).description(msg.desc.size).messages(msg.err.size.invalid)
	);
	...
	return Joi.object({
		generate: Joi.array().required().items(
			Joi.alternatives().try(
				// If SVG input image
				Joi.object({
					from: svgFileSchema.description(msg.desc.from).messages(msg.err.from.invalid),
					to: Joi.array().required().items(Joi.object({
						path: Joi.alternatives().try(
							svgFileSchema.description(msg.desc.path).messages(msg.err.path.invalid),
							rasterFileSchema.description(msg.desc.path).messages(msg.err.path.invalid)
						),
						size: sizeSchema
					})),
					options: Joi.object({
						optimize: Joi.boolean().description(msg.desc.optimize).default(false).messages(msg.err.optimize.invalid),
					}).optional().default({})
				}),

				// If raster input image
				Joi.object({
					from: rasterFileSchema.description(msg.desc.from).messages(msg.err.from.invalid),
					to: Joi.array().required().items(Joi.object({
						path: rasterFileSchema.description(msg.desc.path).messages(msg.err.path.noSvgFromRaster),
						size: sizeSchema
					})),
